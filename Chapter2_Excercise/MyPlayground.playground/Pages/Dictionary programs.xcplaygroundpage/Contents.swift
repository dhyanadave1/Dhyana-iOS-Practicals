...
for key  in encodedMessage {
    if let key = code.first(where: {($0.value == String(key))})?.key {
...
func getWeakestStudent(people : [[String : Any]]) {
    ...
    var fullName = ""
    for key in people {
        if let val = key.first(where: { $0.key == "score" && ($0.value as? Int) ?? 0 < score})?.value {
...
func getFrequency() {
    ...
    for index in 0...numbers.count-1 {
        ...
        //dict[numbers[index]]! += 1;
        if let freq = dict[numbers[index]] {
            dict[numbers[index]] = freq + 1
        } else {
            dict[numbers[index]] = 1
        }
    }
